...
export const checkOtpRestrictions = async (email: string) => {
    if (await redis.get(`otp_spam_lock: ${email}`)) {
        throw new ValidationError("Too many failed OTP attempts! Please try again in 1 hour.");
    }
...
export const verifyOtp = async (email: string, otp: string) => {
    ...
    if (storedOtp !== otp) {
        ...
        if (newCount >= 3) {
            ...
            throw new ValidationError("Too many failed attempts. Please try again in 1 hour.");
        }

        await redis.set(otpCountKey, newCount, "EX", 3600);
        throw new ValidationError(`Incorrect OTP. You have ${3 - newCount} attempt(s) remaining.`);
    }
